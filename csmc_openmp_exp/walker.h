...
public:
    Walker(unsigned, unsigned, unsigned, double, double, Wavefunction<walker>&, const Hamiltonian<walker>&);
    ~Walker();
...
*/
template <typename walker>
Walker<walker>::Walker(unsigned nthreads, unsigned nlevels, unsigned np, double dt, double seed, Wavefunction<walker>& rwf, const Hamiltonian<walker>& rH)
: levels(nlevels), NP(np), delta_t(dt), wf(rwf), H(rH), rgenerators(nthreads, CRandomMersenne(seed)) {
...
#pragma omp parallel for
            for(size_t w = 0; w < walkers.size(); ++w) {
                ...
                walkers[w].second *= delta_t / double(n);
        ...
        if(walkers[0].second < minbag) { break; }
...
void Walker<walker>::random_mv(unsigned& ri, unsigned& rj, std::vector<walker>& rstate) { // inputs will be modified!
    ...
    do {
        ...
        bit = rstate[index1] & (walker(1) << (ri - index1 * nbits)); //CHECK!
    ...
    do {
        ...
        bit = rstate[index2] & (walker(1) << (rj - index2 * nbits)); //CHECK!
    } while(bit != 0); // select final level that is unoccupied
    
    // This moves the pair from the init level to the fin level.
    rstate[index1] ^= walker(1) << (ri - index1 * nbits); //CHECK!
    rstate[index2] ^= walker(1) << (rj - index2 * nbits); //CHECK!
